...
import static org.find9.purge.handlers.MapHandler.mapLandscape;
import static org.find9.purge.handlers.PlayerCooldown.*;
import static org.find9.purge.handlers.PlayerResolver.*;
...
public class MyEventHandler implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event){
        if(event.getAction() == Action.PHYSICAL){
            ...
            if(!getPlayersGroup(event.getPlayer().getUniqueId()).getKey().equals(claim.getKey()) && !isPurge()){
                event.setCancelled(true);
            }
        }
